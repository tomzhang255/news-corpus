...
if __name__ == '__main__':
    ...
    j = 0
    with pd.read_csv(f'/n/henrich_lab/Lab/clean/res6a.csv', dtype=str, chunksize=1000000, encoding_errors='ignore', lineterminator='\n') as reader:
        for df in reader:
            try:
                print('===== a chunk =====')
                print(f'chunk {j}')
                ...
                # define vocabulary
                vocab = pd.read_csv('data/populist_keywords.csv')['word'].to_list()
                ...
                res_curr_chunk.to_csv(f'/n/henrich_lab/Lab/dic4/dic4-{i}-{j}.csv')
                del res_curr_chunk
    ...
    # res.to_csv(f'/n/henrich_lab/Lab/dic4/dic4-{i}.csv', index=False)
