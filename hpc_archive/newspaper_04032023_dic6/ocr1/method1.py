...
def correct(nonword_error, df_coha=None):
    """
    A helper function at the correction step for one single non-word error.
    """
    # TODO take sample, then compute distance
    df_coha = df_coha.sample(frac=0.5, random_state=42)
...
def post_ocr_processing(ocr_text, df_coha, coha_set):
    """
    ...
    df_ocr_text['not_in_set'] = df_ocr_text['word'].apply(
        lambda word: word not in coha_set if random.random() > 0.5 else False)  # reduce # of errors we choose to correct
    ...
    df_ocr_text.loc[df_ocr_text['nonword_error'],
                    'corrected'] = df_ocr_text.loc[df_ocr_text['nonword_error'], 'word'].apply(correct, df_coha=df_coha)
...
if __name__ == '__main__':
    ...
    # play with one piece of text from df_all_ocr
    ocr_text = df_all_ocr.loc[3648, 'text']

    # process this piece of text
    t1 = datetime.now()
    res = post_ocr_processing(ocr_text, df_coha, coha_set)
    print(' '.join(res.corrected))
    t2 = datetime.now()

    # statistics
    print('====================')
    print(f'# words: {res["word"].shape[0]}')
    print(f'# non-word errors: {res["nonword_error"].sum()}')
    print(
        f'Non-word error percentage (out of entire text): {res["nonword_error"].sum() / res.shape[0]}')
    res['correction_took_place'] = res['word'] != res['corrected']
    print(
        f'Correctable error percentage (out of all non-word errors): {res["correction_took_place"].sum() / res["nonword_error"].sum()}')
