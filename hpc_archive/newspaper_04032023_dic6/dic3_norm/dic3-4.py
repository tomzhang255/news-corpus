...
if __name__ == '__main__':
    ...
    j = 0
    with pd.read_csv(f'/n/henrich_lab/Lab/clean/res{i}.csv', dtype=str, chunksize=1000000, encoding_errors='ignore', lineterminator='\n') as reader:
        for df in reader:
            try:
                print('===== a chunk =====')
                print(f'chunk {j}')
                ...
                res_curr_chunk.to_csv(f'/n/henrich_lab/Lab/dic3_norm/dic3-{i}-{j}.csv')
                del res_curr_chunk
    ...
    # res.to_csv(f'/n/henrich_lab/Lab/dic3_norm/dic3-{i}.csv', index=False)
