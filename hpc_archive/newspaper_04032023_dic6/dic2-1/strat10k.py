# for each year, sample about 10k documents
# sampling with replacement because some years have < 100 documents
...
np.random.seed(42)
stratified_sample = meta1.groupby('Year').apply(lambda x: x.sample(n=10000, replace=True))
...
collect()
res.to_csv(base_clean + 'strat_sample_10k.csv', index=False)
