...
def not_in_set(word):
    if random.random() > 0.5:  # TODO correct only 50% non-word errors
...
def correct(nonword_error):
    ...
    df_coha1 = df_coha.sample(frac=0.5, random_state=42)

    # calculate Damerau-Levenshtein (DL) distance between non-word error and each coha word
    df_coha1['dist_with_curr_error'] = df_coha1['word'].apply(lambda w: distance(nonword_error, w))
    ...
    df_coha1['is_candidate'] = df_coha1['dist_with_curr_error'] <= thresold

    if df_coha1['is_candidate'].sum() == 0:
        # uncorrectable, not processed to a correction, return original error word
        return nonword_error

    df_candidates = df_coha1.loc[df_coha1.is_candidate].reset_index(drop=True).copy()
...
if __name__ == '__main__':
    ...
    df_all_ocr = pd.read_csv(OCR_FILE_PATH)
    ...
    df_all_ocr['text'] = pd.Series(res, index=df_all_ocr.index)
    df_all_ocr.to_csv('res5k.csv', index=False)
