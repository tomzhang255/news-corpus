...
if __name__ == '__main__':
    ...
    j = 0
    with pd.read_csv(f'/n/henrich_lab/Lab/ocr/ocr{i}.csv', dtype=str, chunksize=500000, encoding_errors='ignore', lineterminator='\n') as reader:
        for df in reader:
            try:
                print('===== a chunk =====')
                print(f'chunk {j}')
                ...
                # define vocabulary
                vocab = pd.read_csv('data/religiosity.csv')['word'].to_list()
                ...
                res_curr_chunk.to_csv(f'/n/henrich_lab/Lab/dic5/dic5-{i}-{j}.csv')
                del res_curr_chunk
