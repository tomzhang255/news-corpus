...
def post_ocr_processing1(ocr_text):
    ...
    df_ocr_text['not_in_set'] = df_ocr_text['word'].apply(
        lambda word: word not in coha_set if random.random() < 0.05 else False)  # reduce # of errors we choose to correct
...
if __name__ == '__main__':
    ...
    print(f'{datetime.now()} - 5% correction done')

    # save
    sub.to_csv('test5.csv', index=False)
