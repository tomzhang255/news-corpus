...
if __name__ == '__main__':
    ...
    t1 = datetime.now()
    res = post_ocr_processing(ocr_text)
    print(res)
    t2 = datetime.now()
    ...
    print(f'Non-word error percentage (out of entire text): {res["nonword_error"].sum() / res.shape[0]}')
    res['correction_took_place'] = res['word'] != res['corrected']
    print(f'Correctable error percentage (out of all non-word errors): {res["correction_took_place"].sum() / res["nonword_error"].sum()}')
