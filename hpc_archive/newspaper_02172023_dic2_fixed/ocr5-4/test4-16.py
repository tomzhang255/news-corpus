...
def post_ocr_processing(ocr_text):
    ...
    df_ocr_text['not_in_set'] = df_ocr_text['word'].apply(
        lambda word: word not in coha_set if random.random() < 0.018 else False)  # reduce # of errors we choose to correct
...
if __name__ == '__main__':
    ...
    df = pd.read_csv(f'{BASE}res{i}.csv', skiprows=start - 1, nrows=size)
    df_all_ocr = df.loc[[df.path_id == '#:\\Harvard\\185\\7168954\\352319514.html'], :]
    ...
    df_all_ocr.to_csv('test4-16.csv', index=False)
